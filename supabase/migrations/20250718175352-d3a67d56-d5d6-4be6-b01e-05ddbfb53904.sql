-- Create referrals table to track referral codes and free trial grants
CREATE TABLE public.referrals (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  referrer_user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  referral_code TEXT UNIQUE NOT NULL,
  used_by_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  free_trial_granted BOOLEAN DEFAULT FALSE,
...
-- Edge functions can insert and update referrals
...
-- Add free_trial_end column to subscribers table
ALTER TABLE public.subscribers 
ADD COLUMN free_trial_end TIMESTAMPTZ;

-- Update subscribers table RLS policies to allow edge functions to update
CREATE POLICY "update_for_free_trial" ON public.subscribers
FOR UPDATE
USING (true);

-- Function to grant free trial
